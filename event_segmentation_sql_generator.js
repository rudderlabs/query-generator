...
function constructWhere(filters){
    ...
    for (var index=0; index < filters.length; index++){
        ...
        if (index > 0){ //add 'and' from second where clause onwards
            wherePart += " and ";
        }
...
app.post('/', function (req, res) {
 ...
 res.status(404).send('Please use one of endpoints - /getquery, /getevents, /geteventproperties, /geteventpropertyvalues');
...
app.post('/getquery', function(req, res) {
	...
	if (queryInput.events && queryInput.events.length > 0){
		...
		for (var index=0; index < queryInput.events.length; index++){
			...
			if (!(queryInput.events[index].name && queryInput.events[index].name.length > 0)){
			...
			var singleQuery = "select count(*) as event_count, cast(sent_at as timestamp) as sent_at ";
			//add the group by columns in select and also construct the group by clause
			if (queryInput.events[index].group_by && queryInput.events[index].group_by.length > 0){
				for (var groupByIndex = 0; groupByIndex < queryInput.events[index].group_by.length; groupByIndex++){
					singleQuery += " , " + queryInput.events[index].group_by[groupByIndex];                
					groupByClause += " , " + queryInput.events[index].group_by[groupByIndex];                
				}
			...
			if (queryInput.events[index].filters && queryInput.events[index].filters.length > 0){
				singleQuery += " where " + constructWhere(queryInput.events[index].filters);
...
app.post('/getevents', function(req,res) {
	...
	var queryInput = req.body;
	//extract prefix to be used for all tables
	if(!(queryInput.database && queryInput.schema && queryInput.account && queryInput.password)){
	...
	try {
		if (fs.existsSync(path)) {
			...
			if (queryInput && queryInput.cache_refresh_hours && (queryInput.cache_refresh_hours >= hours)){
...
app.post('/geteventpropertyvalues', function(req,res) {
	...
	var query = "select distinct(" + columnName + ") from " + tableName + " limit 100 ";
